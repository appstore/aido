...
```
截图 → Ctrl+C → aiclip --preset ocr --copy → Ctrl+V
...
aiclip --preset ocr --copy

# 翻译（英文→中文，其他语言→英文）
aiclip --preset translate --copy

# Code review
git diff | aiclip --preset code-review

# 摘要
aiclip --preset summarize

# 本地模型（vLLM / SGLang / llama.cpp / Ollama / LM Studio）
git diff | aiclip --base-url http://localhost:30000 -m qwen3 --preset code-review

# 管道传图片（无需剪贴板，可用于无头环境）
aiclip < screenshot.png --preset ocr --copy

# 指定完整 prompt
pbpaste | aiclip -p "改成正式商务邮件语气" --copy
```
...
| 平台 | 路径 |
|---|---|
| Linux | `~/.config/aiclip/config.toml` |
| macOS | `~/Library/Application Support/aiclip/config.toml` |
| Windows | `%APPDATA%\aiclip\config.toml` |

（环境变量 `AICLIP_CONFIG` 可指定其他路径；指定的文件必须存在，否则启动即报错。）
...
| 变量 | 说明 |
|---|---|
| `AICLIP_CONFIG` | 配置文件路径（必须指向已存在的文件） |
| `AICLIP_PROFILE` | 默认 profile |
| `AICLIP_BASE_URL` / `OPENAI_BASE_URL` | 接口地址 |
| `AICLIP_API_KEY` / `OPENAI_API_KEY` | API key |
| `AICLIP_MODEL` | 模型名 |
| `AICLIP_MAX_TOKENS` | 最大生成 token 数（`0` 表示不发送该字段） |
| `AICLIP_TEMPERATURE` | 采样温度 |
| `AICLIP_PRESETS_DIR` | 自定义 preset 目录 |
...
自定义 preset：在配置目录下的 `presets/` 放 `NAME.toml`（文件名即 preset 名，Linux 为 `~/.config/aiclip/presets/`，其他平台见上文配置路径表；可用 `AICLIP_PRESETS_DIR` 指定其他目录）。例如润色工具 `polish.toml`：
...
- `aiclip --list-presets` 可随时核对最终生效的完整列表。

## Linux 剪贴板说明

X11（以及多数 Wayland 合成器）的剪贴板内容依附于写入它的进程，进程退出后内容即失效。aiclip 写剪贴板时会自动派生一个后台子进程，把内容保持一段时间（默认 45 秒，`settings.hold_secs` 可调），行为与 `xclip` / `wl-copy` 一致。
